#A pair of functions that first makes a cacheable square matrix 
#and then solves the matrix for its inverse but only after checking 
#if the very same matrix has been solved for before using this function, 
#thus has a cached solved inverse in memory.
...
makeCacheMatrix <- function(x = matrix()) {		#input x is defined as matrix
		...
		i <- NULL		#i will hold value for inverse matrix. 
						#Reset to NULL each time makeCacheMatrix is called.
